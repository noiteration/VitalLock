...
class Block {
	constructor(index, timestamp, data, previousHash = "") {
		...
		this.date = new Date().toISOString(); // Add this line
		this.data = data;
	...
	calculateHash() {
		return crypto
			.createHash("sha256")
			.update(this.index + this.previousHash + this.timestamp + this.date + JSON.stringify(this.data)) // Include this.date in the hash calculation
			.digest("hex");
	}
}
...
class Blockchain {
	...
	getUserData(userId) {
		return this.chain
			.filter((block) => block.data.user_id === userId)
			.map((block) => ({
				data: block.data,
